"""Local CSV connector — wraps blocking pandas I/O in asyncio.to_thread."""
...
class CSVConnector(BaseConnector):
    """Read and adaptively sample a local CSV file without blocking the event loop."""
    ...
    async def sample(self, target_col: Optional[str] = None) -> pd.DataFrame:
        """Return a sampled DataFrame from the CSV file."""
        await self.connect()
        try:
    ...
    def _count_rows(self) -> int:
        """Fast row count via binary line scan (excludes header)."""
        with open(self._source.path, "rb") as fh:
